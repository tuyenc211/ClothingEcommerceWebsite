...
export default function ParentCategoryPage() {
  ...
  const childCategories = useMemo(() => {
    if (parentCategory) {
      return getChildCategories(parentCategory.id).filter(cat => cat.isActive);
    }
    return [];
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Filter by parent category and its children
    if (parentCategory) {
    ...
    return filtered;
  }, [getPublishedProducts, parentCategory, childCategories, searchTerm, filters, searchProducts]);
  ...
  const handleAddToCart = (productId: number) => {
    ...
    if (product && product.variants && product.variants.length > 0) {
      // Use first available variant for demo
      const firstVariant = product.variants[0];
      addToCart(firstVariant, 1);
      alert(`Đã thêm "${product.name}" vào giỏ hàng!`);
    } else {
      alert("Sản phẩm không có phiên bản khả dụng");
    }
  };
            ...
            <ol className="inline-flex items-center space-x-1 md:space-x-3">
              ...
              <li>
                <div className="flex items-center">
                  ...
                  <Link href="/categories" className="text-gray-500 hover:text-gray-700 ml-1 md:ml-2">
                    Danh mục
                  </Link>
                </div>
              </li>
